...
export default function HoursOfService({ params }) {
  ...
  const handleToggleDate = async (index, date = null) => {
    ...
    // If expanding and no data is present, fetch the graph data
    if (newExpandedRows.has(index) && date && !graphDatas?.[index]) {
      try {
        await GraphData(date, index);
      } catch (error) {
        console.error(`Failed to fetch graph data for index ${index}:`, error);
      }
    }
  };
        ...
        <table className="mt-6 table gs-7 gy-7 gx-7 d-table">
          <thead>
            <tr className="fs-7">
              ...
              <th className="text-start">Co Driver</th>
            </tr>
          </thead>
          <tbody>
            {Array(5).map((_, index) => (
  ...
  return (
    <div className="container-fluid hos-head-top">
      <Link
        href={`/dashboard/drivers/detail/${driverId}`}
        className="align-items-start flex-column btn btn-outline btn-outline btn-outline-muted btn-active-light-secondary"
      >
        ...
        >
          <div className="p-4">
            <input
              ...
              value={`${startDateFormatted} - ${(endDateFormatted > formatDates(today)) ? formatDates(today) : endDateFormatted}`}
              style={{
                padding: "10px",
                textAlign: "center",
      ...
      {isDateKeyLoading ? (
        <div className="table-responsive">
          <table className="mt-6 table gs-7 gy-7 gx-7 d-table hoverable-table" style={{ verticalAlign: 'middle' }}>
            <thead>
              <tr className="fs-7">
              ...
              {finalData?.map((row, index) => (
                <React.Fragment key={index}>
                  ...
                  >
                    <td className="text-start fw-bolder">
                      <i
                        className={`ki-duotone ${isDateOpen.has(index) || isAllOpen ? "ki-up" : "ki-down"} fs-5 me-2 fw-bolder`}
                      ></i>
                      {row.date}
                    </td>
                    <td className="text-center">{row.shift}</td>
                    <td className="text-center">{row.driving}</td>
                    <td className={`text-center ${row.inViolation !== "00:00:00" ? "text-danger" : ""}`}>
                    ...
                    <td className="text-center">{row.logs ? row.logs[9] : ".........."}</td>
                    <td className="text-center">{row.logs ? row.logs[10] : ".........."}</td>
                    <td className="text-center">
                      {row && (
                        <i className={`ki-duotone ${row} me-2 align-middle alert-danger`}>
                        ...
                        <div className="border rounded-3 shadow">
                          <div className="text-start m-5 fs-6">
                            <div>
                              <span className="fw-medium">
                                Carrier Name:{" "}
                                <span className="fw-normal">{row.logs[1][3]?.career_name}</span>
                              </span>
                            </div>
                            <div>
                              <span className="fw-medium">
                          ...
                          <div className="text-start m-5 fs-6">
                            <div>
                              <span className="fw-medium">
                                Home Terminal Name:{" "}
                                <span className="fw-normal">{row.logs[1][3]?.home_terminal_name}</span>
                              </span>
                            </div>
                            <div>
                              <span className="fw-medium">
                                Home Terminal Address:{" "}
                                <span className="fw-normal">{row.logs[1][3]?.home_terminal?.name}</span>
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="m-5">
                            ...
                            {graphDatas ? (
                              <Suspense fallback={<Skeleton height={200} />}>
                                <MemoizedLineChart params={graphDatas[row.date]} />
                              </Suspense>
                            ) : (
                                  ...
                                  row.datas.map((datas, i) => {
                                    ...
                                    return (
                                      <tr className="fs-7" key={i}>
                                        <td className="text-center">
                                          <span className="fs-6 fw-semibold">{datas[4]} -</span>
                                          <div><span>{datas[5]}</span></div>
                                        </td>
                                        <td className="text-center">{datas[0]}</td>
                                        <td className="text-center">
                                          <span className="badge text-bg-dark fs-7">{datas[1]}</span>
                                        </td>
                                        <td className="text-center">{datas[2]}</td>
                                        <td className="text-center">{datas[3]}</td>
                                        <td className="text-center">{datas[7]}</td>
                                        <td className="text-center">{datas[8] || "......"}</td>
                                      </tr>
                                    );
                                  })
                                ) : (
                              ...
                              <div className="text-start">
                                ...
                                {row?.total?.map((section, sectionIdx) =>
                                  section.map((datas, dataIdx) => (
                                    <div key={`${sectionIdx}-${dataIdx}`} className="ms-8 mt-2 fs-7">
