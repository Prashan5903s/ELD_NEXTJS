...
import AddalertModal from "./alertForm/AddalertPopup";
...
import ToggleSwitchAlert from "../../../Components/alertToggle";
...
const AlertTable = () => {
  ...
  // This api is used for fetching permissions data
  ...
  const fetchVehicles = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`${url}/alert/user/info`, {
  ...
  const columns = useMemo(
    () => [
      { header: "Name", accessorKey: "name" },
      {
        ...
        accessorKey: "",
        cell: (info) => {
          const { type, selected } = info.row.original;
          return (
      ...
      },
      {
        ...
        accessorKey: "",
        cell: (info) => {
          const { paragraph, method } = info.row.original;
          return (
            <div className="d-flex flex-column flex-wrap">
              <div>{paragraph}</div>
              <div>{method == 1 ? "Email & notification" : (method == 2 ? ("Notification only") : ("Email only"))}</div>
            </div>
          );
        }
      },
    ...
    <>
      ...
      <div className="listItems">
        <div className="topBar">
          <div className="title">
            <h2>Alert List</h2>
            <div className="path">Dashboard Alert</div>
          </div>
        </div>
