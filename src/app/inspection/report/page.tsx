...
const Events = () => {
  ...
  const fetchFilterData = useCallback(
    debounce(async () => {
      ...
      try {
        const response = await axios.get(`${url}/data/report/inspection/${selectEventDatas[0]}/${selectEventDatas[1]}/${selectEventDatas[2]}/${selectEventDatas[3]}`, {
  ...
  return (
    <>
      ...
      <EventsTable data={(filterData)} loading={loading} type={type} timeNow={timeNow} types={types} setTypes={setTypes} />
    </>
  );
};
