...
export const EventsFilters = ({
  ...
  useEffect(() => {
    if (date_start && date_end) {
      setEventDatas([date_start, date_end, selectedDriver, selectedSafetyScore]);
    }
  }, [
    date_start,
    date_end,
  ...
  useEffect(() => {
    if (data) {
      ...
      setFilterData([drivers, vehicles, pointName]);
  ...
  const handleSafetyScoreFilterChange = (selectedDrivers: number[]) => {
    if (!selectedDrivers || selectedDrivers.length === 0) {
      selectedDrivers = null; // Set to null if no drivers are selected
    }
    setSelectedSafetyScore(selectedDrivers);
  };
              ...
              <Filter
                data={
                  filterData && filterData.length > 0
                    ? filterData[2] && filterData[2]
                    : []
                }
                value={selectedSafetyScore}
                label="Safety score"
                placeholder="Search safety score"
                onFilterChange={handleSafetyScoreFilterChange}
              />
            </div>
            <div>
              <Filter
